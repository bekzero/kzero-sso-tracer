...
): { nameId?: string; nameIdFormat?: string } => {
...
export const decodeSamlArtifact = (
  encoded: string,
  ...
  try {
    ...
    // Populate canonical ACS URL from AuthnRequest if present
    artifact.assertionConsumerServiceURL = findValue(parsed, '@_AssertionConsumerServiceURL');
    if (artifact.assertionConsumerServiceURL) {
      // Temporary diagnostic for runtime verification
      // eslint-disable-next-line no-console
      console.debug(
        '[diag] ACS URL parsed from AuthnRequest (live):',
        artifact.assertionConsumerServiceURL
      );
    }
    // Fallback: if not found by parsing, try to extract from the raw decoded XML as a secondary signal
    if (!artifact.assertionConsumerServiceURL && typeof artifact.decodedXml === 'string') {
      const m = artifact.decodedXml.match(/AssertionConsumerServiceURL\s*=\s*"([^"]+)"/i);
      if (m && m[1]) artifact.assertionConsumerServiceURL = m[1];
    }
