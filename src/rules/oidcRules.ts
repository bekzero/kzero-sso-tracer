...
export const runOidcRules = (events: NormalizedEvent[]): Finding[] => {
    ...
    if (discovery.issuer !== expected) {
      findings.push(
        makeFinding({
          ...
          explanation:
            'The issuer in discovery does not match the tenant endpoint used for discovery.',
          observed: discovery.issuer,
          expected,
          evidence: [discovery.url],
          action:
            'Compare tenant casing and ensure Discovery Endpoint and Issuer values match exactly.',
          confidence: 0.92,
  ...
  if (discovery?.artifacts?.discoveryError) {
    findings.push(
      makeFinding({
        ...
        evidence: [discovery.url],
        action:
          'Verify the discovery URL returns valid JSON. Check for proxy/gateway that may be returning error HTML instead of JSON, or that the URL is correct for your IdP.',
        confidence: 0.94,
  ...
  if (discovery?.artifacts?.discovery) {
    ...
    const authEndpoint =
      typeof disc.authorization_endpoint === 'string' ? disc.authorization_endpoint : null;
    ...
    if (hasPrivateEndpoint) {
      findings.push(
        makeFinding({
          ...
          evidence: [discovery.url],
          action:
            'Verify IdP endpoints are publicly accessible. Internal-only endpoints will fail for cloud/SaaS vendors.',
          confidence: 0.58,
          isAmbiguous: true,
          ambiguityNote:
            'Internal-looking endpoints could be valid for VPN-based setups or private IdP deployments. Verify this matches your deployment model.'
        })
      );
    } else if (issuerHost) {
      ...
      if (suspiciousHostFound) {
        findings.push(
          makeFinding({
            ...
            explanation:
              'The authorization_endpoint, token_endpoint, or jwks_uri host does not match the issuer host family.',
            observed: 'Endpoint hosts differ across incompatible families',
            expected: 'Consistent host family across issuer and endpoints',
            evidence: [discovery.url],
            action:
              'Verify all endpoint URLs use the expected host family. Mismatched hosts may indicate copied values from another environment or cross-environment configuration.',
            confidence: 0.72,
            isAmbiguous: true,
            ambiguityNote:
              'Endpoint host differences could indicate multi-tenant setup, CDN, reverse proxy, or actual misconfiguration. Check if this is expected for your IdP.',
    ...
    if (observed !== expected) {
      findings.push(
        makeFinding({
          ...
          explanation:
            'The callback URL reached by the browser does not match the requested redirect_uri.',
          observed,
          expected: authorize.redirectUri,
          evidence: [authorize.url, callback.url],
          action:
            'Align redirect URI values exactly on both sides (scheme, host, path, trailing slash).',
          confidence: 0.95,
  ...
  if (errSource?.error) {
    ...
    findings.push(
      makeFinding({
        ...
        action:
          'Use the exact error to align client credentials, scopes, and supported response settings.',
        confidence: 0.97
      })
    );
  }
    ...
    if (!authorize.state || !callback.state || authorize.state !== callback.state) {
      findings.push(
        makeFinding({
          ...
          observed: `authorize=${authorize.state ? mask(authorize.state, 2, 2) : '(missing)'}, callback=${callback.state ? mask(callback.state, 2, 2) : '(missing)'}`,
          expected: 'Matching non-empty state values',
          evidence: [authorize.url, callback.url],
          action:
            'Ensure vendor preserves state through redirects and no browser plugin strips query/fragment values.',
          confidence: 0.91
        })
      );
    }
    ...
    if (authorize.responseType?.includes('id_token') && !authorize.nonce) {
      findings.push(
        makeFinding({
          ...
          explanation:
            'Nonce should be present when id_token is returned from the authorization endpoint.',
  ...
  if (authorize?.codeChallenge && token && !token.codeVerifier) {
    findings.push(
      makeFinding({
        ...
        explanation:
          'Authorization used a code_challenge but token exchange did not include code_verifier.',
    ...
    if ([401, 403, 408, 429, 502, 503, 504].includes(jwks.statusCode)) {
      findings.push(
        makeFinding({
          ...
          evidence: [jwks.url],
          action:
            'Validate DNS, TLS chain, and WAF allow rules for vendor to reach KZero endpoints.',
          confidence: 0.83
        })
      );
    }
  ...
  if (
    token?.idToken?.payload?.iss &&
    discovery?.issuer &&
    token.idToken.payload.iss !== discovery.issuer
  ) {
    findings.push(
      makeFinding({
  ...
  if (callback?.code && (!token || (token.statusCode ?? 0) >= 400 || token.error)) {
    findings.push(
      makeFinding({
        ...
        observed: token
          ? `token status ${token.statusCode ?? 'unknown'}`
          : 'no token call captured',
    ...
    if (
      ...
      findings.push(
        makeFinding({
          ...
          explanation:
            'Observed token request style does not align with declared auth method clues.',
          ...
          evidence: [token.url],
          action:
            'Compare Client authentication setting with vendor token auth method implementation.',
          confidence: 0.7
        })
      );
    }
  ...
  if (!authorize && callback) {
    findings.push(
      makeFinding({
        ...
        isAmbiguous: true,
        ambiguityNote:
          'This is informational - the flow may have succeeded, but capture started after the authorize was sent. This usually means capture started late, the initial authorize happened outside the captured tab/window, or the authorize request was not retained.',
  ...
  if (!authorize && !callback && hasStrongDownstreamActivity(oidc)) {
    ...
    findings.push(
      makeFinding({
        ...
        explanation:
          'Token, userinfo, or logout activity was captured without the authorize request.',
        observed:
          'Callback or token activity was captured, but the authorize request was not present in this trace.',
        expected: 'Authorize request present in trace',
        action:
          'This usually means capture started late, the initial authorize happened outside the captured tab/window, or the authorize request was not retained. If SP-initiated flow is expected, start capture before initiating login.',
        evidence: downstreamEvent ? [downstreamEvent.url] : [],
        confidence: 0.58,
        isAmbiguous: true,
        ambiguityNote:
          'This usually means capture started late, the initial authorize happened outside the captured tab/window, or the authorize request was not retained.',
        traceGaps: ['Authorize request not captured'],
  ...
  if (authorize && !callback && !correlation.lateCapture) {
    findings.push(
      makeFinding({
        ...
        action:
          'Verify the app received a callback to the configured redirect URI, including form_post if that response_mode is in use. Check for browser extension/CSP blocking or vendor redirect handler issues.',
        evidence: [authorize.url],
        confidence: 0.68,
  ...
  if (authorize && authorize.responseType?.includes('code') && !authorize.codeChallenge) {
    findings.push(
      makeFinding({
        ...
        explanation:
          'response_type=code was used but no code_challenge was present in the authorize request.',
        observed: 'response_type=code but no code_challenge present',
        expected: 'PKCE code_challenge for Authorization Code flow',
        action:
          'Enable PKCE (Use PKCE) in vendor OIDC client - recommended for all Authorization Code flows for security.',
        evidence: [authorize.url],
        confidence: 0.74,
  ...
  if (authorize?.codeChallengeMethod && authorize.codeChallengeMethod !== 'S256') {
    findings.push(
      makeFinding({
        ...
        action:
          'Set code_challenge_method to S256 in vendor OIDC client. Plain method is considered weak and not recommended.',
        evidence: [authorize.url],
        confidence: 0.84,
    ...
    if (missingFields.length > 0) {
      findings.push(
        makeFinding({
          ...
          action:
            'Verify discovery document structure - may indicate stale copied values, wrong discovery URL, or malformed IdP metadata. Check all required fields are present and properly formatted.',
          evidence: [discovery.url],
          confidence: 0.86,
  ...
  if (userinfo && (userinfo.statusCode ?? 0) >= 400) {
    findings.push(
      makeFinding({
        ...
        action:
          'Check UserInfo endpoint accessibility, token validity, and vendor UserInfo URL configuration.',
    ...
    if (recentAuthorizes.length > 2) {
      findings.push(
        makeFinding({
          ...
          explanation:
            'Multiple authorization requests were captured without any callback, which may indicate the browser is blocking cookies or storage.',
          observed: `${recentAuthorizes.length} authorize requests without callback`,
          expected: 'Single authorize → callback flow',
          action:
            'Check browser for blocked cookies or storage - common with incognito mode or privacy extensions.',
          evidence: recentAuthorizes.map((e) => e.url),
          confidence: 0.52,
          isAmbiguous: true,
          ambiguityNote:
            'Pattern suggests cookie/storage blocking, but could also be vendor redirect loop. Verify in browser devtools.',
          traceGaps: ['Callback event captured'],
          disqualifyingEvidence: [
            'Callback captured after authorize',
            'Single authorize → callback flow observed'
          ]
        })
      );
    }
  ...
  if (callback?.code && !landing.detected) {
    findings.push(
      makeFinding({
        ...
        explanation:
          'OIDC callback with authorization code was captured, but no app landing was detected within the capture window.',
        ...
        action:
          'Check if vendor callback handler properly redirects to app landing page. Verify redirect URI points to correct app URL.',
        evidence: [callback.url],
        confidence: 0.64,
        isAmbiguous: true,
        ambiguityNote:
          'Landing may have happened but was filtered as noise, occurred outside capture window, or SPA routing finished without full navigation event. This is a clue, not a definitive failure.',
