...
const normalizeUrl = (urlStr: string): string => {
  try {
    ...
    const sortedParams = [...new URLSearchParams(url.search)].sort((a, b) =>
...
const isLikelyDocumentLanding = (event: NormalizedSamlEvent): boolean => {
  try {
    ...
    if (pathname.includes('/ws/') || pathname.includes('/chat/') || pathname.includes('/status'))
      return false;
...
const detectSamlFlow = (
  ...
  success: 'clear' | 'probable' | 'none';
  captureCompleteness: 'complete' | 'late' | 'unknown';
} => {
...
export const runSamlRules = (events: NormalizedEvent[]): Finding[] => {
  ...
  // but suppresses it when a more specific ACS mismatch is already present for
  // the same event context (see ACS_RECIPIENT_MISMATCH handling below).
  if (requestEvent && !responseEvent && kzeroSamlEndpoint4xx) {
    ...
    findings.push(
      makeFinding({
        ...
        explanation:
          'The service provider sent an AuthnRequest, but KZero returned an error before login completed and before any SAMLResponse was produced. Verify the ACS URL in the request matches the configured SAML Reply URL.',
        ...
        action: `Open the KZero integration and compare these values: (1) Requested ACS URL from the trace: ${acs}, (2) KZero Valid Redirect URIs, (3) KZero Assertion Consumer Service POST Binding URL. They must match exactly. Check for wrong hostname, wrong tenant, wrong environment, extra slash, or outdated copied URL.`,
        confidence: 0.98,
  ...
  if (!requestEvent) {
    ...
    if (isSuccessful) {
      findings.push(
        makeFinding({
          ...
          isAmbiguous: true,
          ambiguityNote:
            "No AuthnRequest was captured. This could mean: (1) capture started after the request was sent, (2) IdP-initiated login that has no AuthnRequest, or (3) redirect binding AuthnRequest that wasn't captured.",
    ...
    if (flow.success === 'clear') {
      ...
      const isProbable = flow.success === 'probable';
      const severity = likelyIdpInitiated ? 'info' : isProbable ? 'info' : 'warning';
      const owner = likelyIdpInitiated ? 'analysis' : 'vendor SP';
      const title = likelyIdpInitiated
        ? 'We did not capture the service provider sign-in request'
        : isProbable
          ? 'Missing SAMLRequest'
          : 'Missing SAMLRequest';
      const explanation = likelyIdpInitiated
      ...
      const action = likelyIdpInitiated
      ...
      const ambiguityNote = likelyIdpInitiated
        ? 'The trace indicates KZero-to-SP direction, so missing AuthnRequest may be normal rather than a configuration error.'
        : isProbable
          ? 'No AuthnRequest captured. Could be SP-initiated with late capture, or IdP-initiated login.'
          : undefined;
      ...
      findings.push(
        makeFinding({
          ...
          confidence: 0.72,
          traceGaps: ['AuthnRequest not captured'],
          isAmbiguous: likelyIdpInitiated || isProbable,
          ambiguityNote,
          disqualifyingEvidence: isProbable
            ? ['Captured AuthnRequest matching this response']
            : undefined
        })
      );
    }
  ...
  if (parseErrorEvent) {
    findings.push(
      makeFinding({
        ...
        observed:
          parseErrorEvent.samlRequest?.parseError ??
          parseErrorEvent.samlResponse?.parseError ??
          'unknown parse error',
  ...
  if (
    ...
    findings.push(
      makeFinding({
        ...
        confidence: 0.96,
        eventId: requestEvent?.id ?? responseEvent.id
      })
    );
  }
  ...
  if (
    ...
    findings.push(
      makeFinding({
        ...
        action:
          'Compare Service provider Entity ID in KZero with vendor SP Entity ID/Audience URI.',
        confidence: 0.95
      })
    );
  }
  ...
  if (responseEvent?.relayState && !requestEvent?.relayState) {
    const flow = responseEvent
      ? detectSamlFlow(samlEvents, responseEvent)
      : { success: 'none' as const, captureCompleteness: 'unknown' as const };
    ...
    findings.push(
      makeFinding({
        ...
        ambiguityNote: isAmbiguous
          ? 'No AuthnRequest was captured, so we cannot verify if RelayState was expected on the request side.'
          : undefined,
        traceGaps: !requestEvent
          ? ['AuthnRequest not captured - cannot verify RelayState round-trip']
          : undefined
      })
    );
  }
  ...
  if (!requestEvent && responseEvent?.samlResponse?.inResponseTo) {
    findings.push(
      makeFinding({
        ...
        explanation:
          'Response carries InResponseTo but no AuthnRequest is present in this capture.',
        observed: `InResponseTo=${responseEvent.samlResponse.inResponseTo}`,
        expected: 'Captured AuthnRequest for SP-initiated flow',
        evidence: [responseEvent.url],
        action:
          'Verify capture start timing and whether the app expects IdP-initiated or SP-initiated login.',
        confidence: 0.64,
        isAmbiguous: true,
        ambiguityNote:
          'Response contains InResponseTo (suggesting SP-initiated) but no AuthnRequest was captured. This could mean capture started late, or this is actually an IdP-initiated flow that includes InResponseTo from a prior SP-initiated request.',
        traceGaps: ['AuthnRequest not captured'],
        disqualifyingEvidence: [
          'Captured AuthnRequest that matches InResponseTo',
          'Confirmed IdP-initiated login without InResponseTo'
        ]
      })
    );
  }
  ...
  if ((responseEvent?.statusCode ?? 0) >= 400 && responseEvent?.samlResponse) {
    findings.push(
      makeFinding({
        ...
        explanation:
          'SP endpoint returned an error while receiving a SAML assertion, often due to cert/signature expectations.',
        observed: `ACS status ${responseEvent.statusCode}`,
        expected: 'ACS accepts assertion and returns success',
        evidence: [responseEvent.url],
        action:
          'Compare Tenant certificate/XML data and Validate signatures settings with vendor certificate configuration.',
        confidence: 0.67
      })
    );
  }
