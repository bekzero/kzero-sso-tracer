...
describe('decodeSamlArtifact', () => {
  ...
  it('parses SAML Redirect deflate payload', () => {
    const xml =
      '<samlp:AuthnRequest ForceAuthn="true" xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol" />';
  ...
  it('parses realistic Zoho-style response with all attributes', () => {
    const xml = `<samlp:Response xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol" xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion" ID="_kze8f3a1b2c3d4e5f6" Version="2.0" Destination="https://accounts.zoho.com/saml/sp/acs" InResponseTo="_oasis-names:tc:SAML:2.0:status:Success">
  ...
  it('parses AuthnRequest with AssertionConsumerServiceURL attribute', () => {
    const xml =
      '<samlp:AuthnRequest AssertionConsumerServiceURL="https://sp.example.com/acs" ForceAuthn="true" xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol" />';
    const compressed = pako.deflateRaw(xml);
    const encoded = Buffer.from(compressed).toString('base64');
    const artifact = decodeSamlArtifact(encoded, 'redirect');
    expect(artifact.assertionConsumerServiceURL).toBe('https://sp.example.com/acs');
  });
